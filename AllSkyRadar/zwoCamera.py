...
def tstasi178(im):
    q0Ax = 678 ; q0Ay = 57
    img0A = im.crop((q0Ax, q0Ay, q0Ax+2010, q0Ay+2010))
    data0A = img0A.convert ('L')
    stat0A = ImageStat.Stat(data0A)
    #print(stat4A.rms[0])
...
def cap(dataz, datay):
    ...
    my_camera.configure(datay, dataz, 85, 65, 50, 50, 3, 1, [0, 0, 3096, 2080], 24, True, "picture" )
...
def plotting_1(imagCropHD1, vs, vm, vju, vsa, vma, vve, aktual_t_f, licznik_1, test2, min_br, max_br):
    ...
    font_c = {'color':  "black", 'size': 16, 'weight': 'bold', 'family': 'monospace', }
    ...
    ax.plot(vm.az,90-(round(math.degrees(vm.alt),1)),'o',markersize=15, markerfacecolor='none', markeredgecolor='#000000', alpha=0.3)
    ...
    cv2.imwrite(dst_tmp, opencvImageHD, [int(cv2.IMWRITE_JPEG_QUALITY), 95])
...
class AsyncWrite(threading.Thread):  
    ...
    def run(self):
        ...
        crop.save(im_name,"JPEG", quality=95)
        ...
        if (int(self.exposure) < 10000000):
            ...
            imagCropHD = crop.resize((1080, 1080),Image.LANCZOS)
...
def Main(): 
    ...
    while True:
        ...
        if int(dataz9B[0]) == 1:
            ...
            arr = cap(int(dataz0[0]), int(dataz1[0]))
            ...
            aktual_t2 = datetime.datetime.now()
            aaaa = datetime.timedelta.total_seconds(aktual_t2-aktual_t1)
            print("Capture done in:", aaaa, "\n")

            if (aaaa < 10):
                time.sleep(10-aaaa)
                print("Sleeping ", (10-aaaa))
